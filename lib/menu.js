...
function menu_item (o_parent, n_order) {
	...
	var $tt=this.a_config[1];	
	if (($tt) && ($tt.search(/^set_visible/) == -1)) {	
          var t1='" href="' + this.a_config[1] + '"'
                              + (this.a_config[2]  ? ' target="'
                              + this.a_config[2] + '"' : '') ;
          /*org var t1='" href="' + this.a_config[1] + '"'
                              + (this.a_config[2] && this.a_config[2]['tw'] ? ' target="'
                              + this.a_config[2]['tw'] + '"' : '') ;*/
	  var t2=this.n_id;
        } else {			      
          var t1='" href="#1"';
	  var t2=this.a_config[1];
	}  

													
          document.write (
		'<a id="e' + o_root.n_id + '_'
			+ this.n_id +'o" class="' + this.getstyle(0, 0) + t1
			...
			+' z-index: ' + this.n_depth + ';" '
			+ 'onclick="return A_MENUS[' + o_root.n_id + '].onclick('
			+ t2 + ');" onmouseout="A_MENUS[' + o_root.n_id + '].onmouseout('
			+ this.n_id + ');" onmouseover="A_MENUS[' + o_root.n_id + '].onmouseover('
			+ this.n_id + ');" onmousedown="A_MENUS[' + o_root.n_id + '].onmousedown('
			+ this.n_id + ');"><div  id="e' + o_root.n_id + '_'
			+ this.n_id +'i" class="' + this.getstyle(1, 0) + '">'
...
function FilterOnSelectedItem(sel_object, form, field_name) { 
  var i, j, k,  vpos;
  var looked_column;
  var looked_text;
  var looked_object;
  ...
  var connect_selects=form._fields;
  var pos=new Array();
  ...
  var f=form._formcounter;
  for (j=0;j < connect_selects.length;j++) {
    for (i=document.forms[f][connect_selects[j]].options.length; i>=0; i--) {
      document.forms[f][connect_selects[j]].options[i]=null;
    }  
  } 
  ...
  for (j=0;j < connect_selects.length;j++) {
    var n=0;
    for (i = 0; i < form._data.length;i++) { 
      if (form._data[i][ vpos ] == looked_text) {
        var ok=1;
        for (k=0;k<document.forms[f][connect_selects[j]].options.length;k++) {
	  if (document.forms[f][connect_selects[j]].options.length >0 )  {
	    if (document.forms[f][connect_selects[j]].options[k].text == form._data[i][j*2+1]) {
	      ok=0;
	    }
	  }  
	}
	if (ok==1) {
          document.forms[f][connect_selects[j]].options[n] = new Option(); 
          document.forms[f][connect_selects[j]].options[n].value = form._data[i][j*2];  
          document.forms[f][connect_selects[j]].options[n].text = form._data[i][j*2+1];  
 	  n++;
	}  
      }	
    }
...
function set_visible(zindex) {
  var $i=0;
  for ($i=0;$i<mfiles.length;$i++) {
    document.getElementById( mfiles[$i] ).style.visibility="hidden";
  }  
  document.getElementById( mfiles[zindex] ).style.visibility="visible";
}
