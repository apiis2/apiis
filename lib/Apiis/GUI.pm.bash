...
#mue    my $query = $self->Query;
    ...
    if ( $self->GUIobj->GUI ne 'Form' ) {
        ...
        if ( $self->GUIobj->$name->ElementType eq 'Bash' ) {
            $statement = $self->GUIobj->$name->Command;
            $statement = '(' . $statement . ')';
        }
    }
    ...
    my @structure;
    my $a;
    if (( $self->GUIobj->$name->ElementType eq 'Sql' ) and ( $statement =~ /^\((.*)\)$/ )) {
    ...
    elsif (( $self->GUIobj->$name->ElementType eq 'Function' ) and ( $statement =~ /^([_a-zA-Z0-9_]*)\((.*)\)$/ )) {
        my $module;
        my $vfunction = $1;
        my @vparam    = split( ',', $2 );
    ...
    elsif ( $self->GUIobj->$name->ElementType eq 'Bash' ) {
        return \(1,2,3);
    }
}
